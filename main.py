...
def waiting_url_step(message):
    ...
    #нижний регистр делает ссылку невалидной search_url = search_url.lower()
...
def send_tracking_urls_list(uid):
    ...
    if not user_tracking_urls_list:
        bot.send_message(uid, 'Вы ничего не отслеживаете.')
        return
...
# # # Get list of tracked ads # # #
def send_found_ads(uid):
    msg = "Лови пять"
    bot.send_message(message.chat.id, msg)


@bot.message_handler(commands=['ad'])
def print_ads(message):
    send_found_ads(message.chat.id)
